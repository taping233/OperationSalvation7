/* ============================================================
 * 搜打撤 v0.19 —— M1 伤害结算引擎（四类伤害体系 + 诅咒状态）
 *
 * 设计者规则（2026-09-01 两批定版，详见 docs/design.md §3）：
 *   1. 攻击伤害：卡面记号 攻（+n），结算 = n + 人物攻击力，可触发流血效果；
 *      目标每有 1 层流血，每次受到攻击伤害时 +1 点。
 *      流血不会自动掉血（不按回合结算），层数无上限、不衰减。
 *   2. 法术伤害：基于法伤加成计算；卡面记号 n'  = n 点法术伤害。
 *   3. 固定伤害：不受任何伤害加成（攻击力/法伤/增伤均无效）；
 *      卡面纯数字 n（无「攻」无角标）即固定伤害，如「射击」2 点固定。
 *   4. 真实伤害：无视一切防御手段（护甲/护盾/格挡/结界）；
 *      卡面记号 n'' = n 点真实伤害。格挡对真实伤害无效。
 *
 * 诅咒状态（设计者 2026-09-02 定版，详见 docs/design.md §3.5）：
 *   流血 bleed   每层：目标受到攻击伤害时伤害 +1（叠层，无上限不衰减，不自动掉血）
 *   中毒 poison  每层：目标回合结束时受到 1 点固定伤害（叠层，不衰减）
 *   冰冻 freeze  目标 1 回合无法行动（轮到它行动时跳过）
 *   沉默 silence 目标 1 回合技能无法生效（主动与被动技能，攻击除外）
 *   破甲 abreak  目标 2 回合内无法减免伤害（格挡/护盾/护甲全部失效）
 *   禁疗 healban 目标 2 回合内无法回复生命
 *   净化：清除身上所有诅咒类型与效果
 *   计时规则（设计者 2026-09-02 定版，v0.23 精确化）：
 *     持续 1 回合 = 在触发当回合结束前生效；持续 n 回合 = 从触发的当回合开始
 *     到第 n 回合结束前都生效（覆盖触发当回合起的 n 个完整回合）。
 *     实现上采用「共享回合钟」：所有计时状态在每回合结束（双方各行动一次）
 *     统一递减 1，递减到 0 解除——因此持续 1 回合的诅咒/祝福能覆盖到
 *     触发当回合的敌方阶段（如当回合挂的冰冻/潜行能挡住该回合的敌人行动）。
 *
 * 祝福状态（v0.20，设计者 2026-09-02 定版，详见 docs/design.md §3.6）：
 *   攻击力增加 atkUp    攻击伤害 +N（本场战斗，可叠加）
 *   法伤增加   spellUp  法术伤害 +N（本场战斗，可叠加）
 *   潜行       stealth  无法成为被攻击对象；不造成伤害便不会破除潜行，
 *                       造成伤害立即破除；一般标回合持续时间，回合到自动失效
 *   免疫伤害   immune   不受到任何伤害（攻击/法术/固定/真实/中毒全部无效）
 *   减伤       reduce   每次受到的伤害 -N（真实伤害无视，同格挡/护盾）
 *   剑仙形态   swordForm    回合开始时额外抽 1 张（本局对战）
 *   自然形态   natureForm   回合开始时额外获得 1 点能量（本局对战）
 *   宇宙形态   cosmosForm   本局对战内所有卡牌变为 1 费
 *   （净化同时是祝福类动作：清除自己所有诅咒）
 *
 * 实现约定（数值已经设计者确认，见 design.md §3）：
 *   攻击伤害 = 卡面攻击值 n（攻（+n）） + 当前攻击力(atk) + 目标流血层数 × 1
 *   法术伤害 = 卡面法术值 n（n'） + 法伤加成(spellPower)
 *   固定/真实 = 卡面值，不加成
 *   防御吸收顺序：护盾(shield) → 护甲(armor) → 生命(hp)；真实伤害跳过全部；
 *   破甲状态下格挡与护盾/护甲吸收一并失效
 *   格挡(defense.guard)：非真实伤害每次结算降为 1 点；真实伤害无视
 * ============================================================ */
(function () {
