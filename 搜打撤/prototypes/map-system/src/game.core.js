 ...
 * ============================================================ */
  ...
  game.heal = function (n) {
    ...
    game.hp += real;
    const p = fxAt();
    FX.float(`+${real} 生命`, p.x, p.y, '#7fdd9c', true);
    SDT.Sound.sfx('heal');
  ...
  function slotInfoHTML(baseData, run) {
    if (!baseData && !run) return '<span class="slot-empty">—— 空档位 ——</span>';
    ...
    const names = baseData
      ...
      : [];
    return `[[icon:hourglass]] <b>游玩时间</b>：${fmtPlayTime(total)}<br>` +
      `[[icon:trophy]] <b>已解锁成就</b>：${names.length ? names.map(esc).join('、') : '暂无'}`;
  }
  ...
  function openSlotPicker() {
    ...
    for (let i = 1; i <= SLOT_COUNT; i++) {
      ...
      let ops;
      if (!exists) {
        ops = `<button class="ov-btn ok" data-act="newSlot" data-slot="${i}">＋ 开新档</button>`;
      } else {
        ops = `<button class="ov-btn ok" data-act="enterSlot" data-slot="${i}">${run ? '[[icon:play]] 继续对局' : '[[icon:home]] 进入存档'}</button>`;
    ...
    UI.showOverlay('[[icon:archive]] 选择存档', `
      <p class="ov-note">三个档位的<b>基地与进度完全独立</b>（资源/仓库/职业/成就/卡背各自保存）。` +
        `有未完成对局的档位会直接回到那一局；没有对局则先进基地，再从基地出发。</p>
      <div class="slot-list">${rows.join('')}</div>
      <div class="ov-btns"><button class="ov-btn" data-act="slotBack">返回</button></div>`, true);
  ...
  function exitToTitle() {
    ...
    if (game.runActive) { openLeaveMenu(); return; }
    saveGame();
    showTitle();
  }
  ...
  function openSettings() {
    ...
    game.state = 'modal';
    UI.showOverlay('[[icon:gear]] 设置', `
      <div class="settings">
        <label class="chk"><input type="checkbox" id="setIndex" ${game.toggles.index ? 'checked' : ''}> 结点编号</label>
        <label class="chk"><input type="checkbox" id="setDev" ${game.devMode ? 'checked' : ''}> 开发者模式（固定骰子 / 卡牌制作）</label>
        <h3 class="set-h">[[icon:gear]] 音频</h3>
        <label class="chk"><input type="checkbox" id="setMusic" ${SDT.Sound.musicMuted ? '' : 'checked'}> 背景音乐</label>
        <label class="chk vol"><span>音乐音量</span><input type="range" id="setMusicVol" min="0" max="100" value="${Math.round(SDT.Sound.musicVolume * 100)}"><b id="setMusicVolVal">${Math.round(SDT.Sound.musicVolume * 100)}</b></label>
        <label class="chk"><input type="checkbox" id="setSfx" ${SDT.Sound.sfxMuted ? '' : 'checked'}> 音效</label>
        <label class="chk vol"><span>音效音量</span><input type="range" id="setSfxVol" min="0" max="100" value="${Math.round(SDT.Sound.sfxVolume * 100)}"><b id="setSfxVolVal">${Math.round(SDT.Sound.sfxVolume * 100)}</b></label>
        <p class="hint">侧边栏的 [[icon:gear]] 按钮为全局静音；这里可分别开关音乐与音效、拖动滑条调音量（自动保存）。</p>
        <h3 class="set-h">危险区</h3>
        <div class="btn-row">
    ...
    const sync = () => {
      UI.el.tglIndex.checked = game.toggles.index;
      UI.el.tglDev.checked = game.devMode;
      UI.el.devTools.hidden = !game.devMode;
    };
    UI.act('wipeNotes', () => { SDT.Notes.clearAll(); rebuildNotes(); UI.log('已清空全部格子备注', 'warn'); });
    UI.act('wipeCards', () => { SDT.Cards.clearAll(); UI.log('已清空卡牌库', 'warn'); });
    UI.act('wipeSave', () => { clearAllSlots(); UI.log('已清空全部三个档位的存档', 'warn'); });
  ...
  // 每局开始：固定携带 5 张初始牌「杀」（同名堆叠，只占 1 格背包）
  ...
  function newRun(mode, picks) {
    ...
    UI.log('掷骰环走，落脚触发事件；外圈门可撤离，深处有祭坛与 BOSS', 'sys');
    grantStarterSha();
    UI.log(`[[icon:cards]] 随身携带初始牌【<b>杀</b>】×${MAP.rules.starterSha}（固定携带 · 不可入库 / 安全格）`, 'sys');
    ...
    openClassChoice();   // 开局二选一职业 + 1 张随机职业卡（与 5 张杀一起）
  }
