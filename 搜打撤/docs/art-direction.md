...
- 角色与场景位图逐张独立调用内置 ImageGen；需要精确小尺寸字形的 UI/应用图标可由项目内确定性绘制脚本直接生成 PNG/ICO。所有最终文件复制到 `game/assets/`，记录 prompt 或绘制规格、尺寸、SHA-256。
...
## 素材家族生产纪律（2026-09-06，源自 gamedev skills / create-game-assets）

- **以已批准的 hero 资产为编辑基准**：新立绘/卡面/图标必须以当前定版的标题资产或角色立绘为参照输入，锁定 调色板、视角、比例、描边、纹理密度 五个不变量；禁止无参照的孤立出图（风格漂移的主要来源）。
- **小批量生产**：每批 3~5 张，实机尺寸下对齐检查后再批量；新素材在游戏内背景上验收（不只看素材图）。
- **确定性归一**：裁剪、尺寸、锚点、压缩（webp q85）按现有管线执行；登记进 `assets/asset-manifest.json`（`npm run assets:audit` 校验授权与遗漏）。
- **灰箱优先**：功能未定的占位素材用灰箱图，不投入成品级绘制。
