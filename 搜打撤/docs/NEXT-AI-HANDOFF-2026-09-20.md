## 当前状态补充：Agent 接口链 + 数据分析系统 + 页面快捷键/控制台四页签（2026-09-25 23:5x +08:00，未提交待拍板）

- 老板 09-25 晚点单四件套（AskUserQuestion 拍板：内置适配器+外部协议 / 全链路 / 浏览器快进+Node 无头批量 / 游戏内数据页+导出），六个切片全部实装。
- **Agent 接口层**（新域 `game/src/agent/` 九文件 + `docs/agent-protocol.md` 协议文档）：`SDT.Agent.{observe,act,step,autoRun,stopRun,waitSettle,policies,llm,recorder,stats,openDataPage}`。observe=结构化局面 JSON（角色/地图合法移动+visited/战斗快照投影/弹层 data-act 动作面+标签）；act 四型动作全走既有命令面（UI._acts 回调直调 / moveTo / SDT.Battle.commands / sys），非法动作结构化报错不抛不改状态。surface 零侵入扫 `#ovBody [data-act]`；两个合成动作 czClose（特写点背景关）/closeOverlay（closeTopOverlayByEsc 同链，必选页拒绝）。settle=签名连续不变+战斗非 busy；**act 后等动作面变化**（防 hideOverlay 210ms 淡出/转场窗重复点击）。熔断：同动作含参数连打 6 次、同位置 500 步收口。
- **快进 turbo**（agent.turbo.js 叶子开关，localStorage `sdt-agent-turbo`）：battle.clock 演出等待短路（isTestRuntime 同款）、moveTo 立即落点（reduced-motion 同款）、节点过场/拾取揭晓/宝箱翻检(780ms)/出发撕封条(520ms)/lab 通关演出 turboMs 归零——只跳演出不跳逻辑。微改既有文件仅加开关：battle.clock.js / game.run.flow.js / game.run.scenes.js / chests.js / game.hub.depart.js / lab.ending.js / game.boot.js（装配行）/ game.session.js（补导出 openSuggestionInbox）/ battle.lifecycle.js（noteBattle 一行）/ death.js·altar.js·game.menu.js·nest.js（recordRun 各一行）。
- **LLM 适配器**（agent.llm.js）：OpenAI 兼容 `/chat/completions`；配置 localStorage `sdt-agent-llm`（Node 侧 SDT_AGENT_BASE_URL/KEY/MODEL 环境变量覆盖）；严格动作 JSON 解析容错（裸 JSON/代码块围栏/内嵌大括号），非法带错误重试 ≤3 次后退化贪心不卡死。**真模型一局待老板配 key 实测**（mock OpenAI 服务端到端 tests/agent-llm.test.js 7/7）。
- **跑局记录+统计**（agent.recorder/stats/data）：终局挂点四处各一行 SDT 晚绑定——death.js finish→('death')、altar.js showExtractDone→('clear'/'extract' 按 bossKilled)、menu abandonRun finish→('abandon')、battle.lifecycle 收尾 noteBattle(win)。**坑：不可订阅 battle:end 总线事件——served>0 会吞掉 onBattleEnd 0-订阅回退**。记录字段：id/ts/driver/outcome/why/seed/mode(nestActive 派生)/class/characterId/maxLayer/turns/coins/hpLeft/durationSec/battles/carried(+sellPrice 价值)/carriedValue。聚合：通关率/撤离率/死亡率/死亡层分布/带出卡牌价值+Top 卡/行动与战斗胜率/按驱动·职业·模式切分。localStorage `sdt-run-records` 环形 200；数据页 JSON/CSV 导出+两步确认清空。
- **批跑器**（`game/tools/agent-batch.mjs`，jsdom 复用 boot.test 补桩手法，vite-node 转 JSON import）：`npx vite-node game/tools/agent-batch.mjs --runs 10 --policy greedy [--mode nest] [--timeout 180] --out output/agent/` 产 runs.jsonl+summary.json。手段：turbo+immediate 关层 shim、rAF 渲染异常吞噬、jsdomError 静音、局间清理（残留战斗 flee+exitToTitle 带不进下一局）。实测单局 7.5-17s。
- **贪心策略要点**（agent.policy.js，LLM/批跑共用 decide 契约）：地图 visited 近零分+回走重罚(-12/-20)+前进加分防横跳；同弹层 >8 步优先收口动作（closeShop/closeBag/evtNext 等）防流连；最近动作 LRU 跨面板记忆（只回看不复点）；注能态 playDirect 正出口；pendingTarget 每目标单试防静默拒打死循环；强制手牌选择必选牌（skip 被引擎 deny）；resetSlots 才碰 newSlot/overwriteSlot，绝不清档。
- **页面快捷键**（ui/quick-nav.js，Alt+键）：1-6 基地六页签 / 7 卡牌库·照相馆 / 8 留言信箱 / 9 跑局数据 / 0 设置；战斗/对局中受保护页面拒绝并日志提示，不绕守卫。
- **控制台四页签**（Ctrl+L；game.run.dev.js 改版 + agent.console.js；winter.css 追加样式）：调试（原资源/发卡/战斗调试，写操作补对局守卫，任意界面可开）/ Agent（LLM 配置表单、贪心·随机·LLM 自动跑局启停、快进开关、observe 预览）/ 数据（跑局统计面板+导出）/ 快捷键（10 入口+合法性+点击直达）。
- **验证**：agent-llm 7/7、agent-recorder 6/6、contracts（含环检测）+architecture-boundaries 绿、语法 200 文件过、lint 新文件零告警、**全量门禁 900/900 全绿**；实机浏览器贪心快进全链路（整备→出征→地图→战斗→死亡→结算→回基地）73 步 **15.8s**（老板两分钟预算内）；数据页/四页签控制台实机渲染过。批跑标准局四轮迭代复验（LRU 上限/兜底记账/熔断键/门回环/局间清理逐项修复后）：**终验 10/10 真局死亡入账，4-15s/局，零空转零假熔断**；死亡层 1-4 分布、战斗胜率 75%、均行动 14。
- **nest 覆盖实况（更正）**：早期「9.9s 死亡入账(mode=nest)」实为误判——那是主图局。真实状态：`--mode nest` 夹具（keys=10+nestUnlocked+牌盒素材 12 张，**必须包 Base.reset 重放**，档位 newSlot 会归零）驱动 Agent 走通 备战选牌→巢穴地图→战斗→符文法阵→符文箱 全链（mode=nest 记录正确入账）；**nest 战斗无死亡终局**（失败=回满血原格重试），唯二终局=撤离结算（showLabSettlement 已挂 clear 记录）或批跑超时。**待办**：贪心基线 2000 步/492s 仍未打死巢主拿不到撤离记录（battle 1919 步拉锯）——属策略强度非接口问题，巢主 32 心+风暴之手 16×2+复活，需要更强策略（LLM/精细贪心）或战斗效率诊断后再验撤离结算入账。
- **未完成/边界**：真 LLM 一局待 key；批量统计样本量与策略质量待观察（贪心属基线，非强度追求）；全部改动未提交待拍板（提交须老板指示）。并行会话 A3 卡牌迁移（cards.js/battle.engine/preview/resolution）全程未碰；它写盘高频触发 Vite full reload 会冲实机验证现场——批量验证走 Node 无头免疫。贪心实测坑八连（LRU 上限须≥单页动作数现 60 / LRU 兜底复用须记账 / act 后须等动作面变化 / 批跑局间须轮询回标题 / 战斗冻结与同键熔断的 noop·战斗回合键区分 / 地图横跳 visited+回走+刚离格三重罚 / mandatory 手牌选择 skip 被拒 / 随机源必须走 Random 服务）已录 memory `soudache-agent-chain-2026-09-25`。
