...
| E | 帧图批量抠图接入 | ✅ 完成（未提交）：老板 09-12 交付三张 GPT 透明底 12 格精灵图（2行×6列，**整图交付出**，取代逐帧交付方案）→ `.tmp/frames-bake/bake-sheets.py` 切片烘焙（谷底切分防切脚/贴边网格线清除/去低alpha雾）→ assets/portraits/frames 39 张 webp（512×760，旧占位帧全部覆盖）：shuangling 12张（atk-wind×3+atk-hit×2）/ baiqi 12张（cast×3）/ lituan 15张（cast 全链×6），idle 均为呼吸级微差双帧循环；SEQ_MS cast 620→800 适配多帧链；254 测试全绿+构建通过+产物核验 39 帧；对照表在 `.tmp/frames-bake/contact-*.png`。敌人不上序列帧是定调 |
