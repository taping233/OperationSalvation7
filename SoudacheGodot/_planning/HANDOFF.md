...
- 批次 6c：**运行管线（后续所有 smoke/录帧通用）**——本机无 Godot 编辑器二进制，运行时=导出模板 exe（build/windows/升格会的的冬日猜想.exe，release template+嵌入 pck，加载 data_SoudacheGodot_windows_x86_64/SoudacheGodot.dll）；旧嵌入 pck 是 9/8 旧 schema 会启动即炸，须用 _planning/evidence/6c-battle/rebuild-embedded-pck.py 重建（已逆向 pck v4 格式）；流程=dotnet build → 拷 .godot/mono/temp/bin/Debug/SoudacheGodot.dll 进 data 目录 → 跑 exe。fps 验收口径：perf-smoke 随整机负载波动（6b 时 menu 156 → 6c 时 134），以 battle 与 menu 恒差判断相对回退，绝对值批次 7b/8 定标。
- 批次 7b：**fps 定标基线（终版口径，替换 6c「环境折损」临时口径）**——条件=模板 exe 管线（rebuild-embedded-pck.py 重建+data 目录新 dll）+ 性能铁律③启动预热并行（WARMUP_OK imported=69 raw=9 约 1.7-1.9s）+ 本机 RTX 3060 Laptop GPU / gl_compatibility / 1440×900 / 2026-09-12，8s 计时窗两轮实测：**menu 119.4/117.4，battle 108.1/99.9，map 112.3/113.5 fps**（--perf-smoke 0 SCRIPT ERROR）。恒差口径：battle 较 menu -8%~-15%，map 较 menu -3%~-5%；后续批次/批次 8 总验收以「同条件恒差」判断回退，绝对值随整机负载浮动属正常（历史参照：6b 时代 menu 156.7、6c-map 时代 147.6，均无预热且整机状态不同）。运行注意：perf-smoke 已受铁律①失焦暂停影响，跑分期间窗口不得失焦（暂停时长会被平移出计时窗，帧数会掉）。
- 批次 8：**发布链终态与三条管线经验**——①**exe 改名安全、rcedit 不安全**：模板 exe 的 data 目录查找只依赖 project.binary 的 `dotnet/project/assembly_name`（=SoudacheGodot），改 config/name（「升格会的的冬日猜想」→「升格会的冬日猜想」，修网页标题「的的」笔误）不影响 .NET 装配装载，exe 文件名也可任意改；但 rcedit 盖章会破坏嵌入 pck 定位（只保留 PE section 表覆盖字节+重写资源树，实测盖章 PE±补丁全失败、原 PE±补丁全通过，四象限对照实验见 rebuild 工具头注），exe 文件图标因此保持 Godot 默认、留给未来真机 export（export_presets.cfg 已配 app.ico/0.53.3/中文 exe 名）。②**project.binary 补丁**：Godot 4.7 导出的 project.binary 是 ECFG 顺序键值流（magic+u32 count+[klen][key][vlen][value]，String value=[type4][len][bytes][pad4]），rebuild 工具已内置 name/version 幂等补丁；**嵌套 Card 元素是 SerializeToElement 默认策略=PascalCase**（外层 DTO 才是 camelCase），手写存档必须区分。③**raw png 图标回退**：无 .import 的 png 在导出环境 ResourceLoader 必哑（icon-paw.png 自 6b'' 起哑火）——WinterUi.Icon 已加 FileAccess(pck 挂载)+Image.LoadPngFromBuffer 回退（GD.Load 首试的 No loader 报错是固有噪音，可忽略），新增自绘图标配 .png 直装即可。取证设施：AppMain `--boot-load-slot=N` 启动读档（证据存档=槽 1，可删）；fps 终测（同 7b 条件两轮）menu 113.5/112.2、map 108.0/107.4、battle 101.5/103.2、hub-deploy 101.9/100.6、hub-stash 94.6/93.1、hub-ach 98.2/97.9，恒差 battle-menu -8~-10.6%、map-menu -4.3~-4.8% 均在 7b 基线区间内=无相对回退（绝对值整体 -4~-5% 为整机负载漂移，各屏同幅）；stash 页较 6b'' 略重（行内卖卡按钮组）属新增控件成本。
- （后续批次在此追加）
