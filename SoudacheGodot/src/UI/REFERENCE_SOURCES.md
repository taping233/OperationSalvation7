...
## wave5 批次 6b' 图鉴/制作坊/hub 页签 ↔ 真源参数对照（2026-09-12）

| 组件/屏区 | 网页真源（搜打撤/game/src + css） | 取用参数（1:1 口径） |
|---|---|---|
| 卡库数据层 | cards.js RARITIES/TYPES/DMG_TYPES(:93-100) + cards.view.js rarityOf 语义 + game.cardslib.js libFiltered(:72-86) | RARITIES=['初始','古朴','稀有','史诗','传说','衍生','职业','棱彩']；TYPES=['武术','法术','生物','道具','装备','事件','能力卡','资源']；rarityOf=能力卡/hero 链→棱彩、衍生牌递归 tokenOf 源（深度护栏 8）；筛选=类型页签+有效稀有度+名称/描述小写子串；排序=cost 升→稀有度降（RARITIES 序靠后在前）→名称 zh localeCompare；分页 24/页（LIB_PAGE_SIZE cardslib.js:60） |
| 卡面控件 CardFace | cards.view.js cardHTML(:28-93) + css/cards.css .hs-card 块(:4-148) | 10em×14em em 布局（fontPx=em 基准）；卡底 linear-gradient(165deg,fc1,fc2 72%)+2px 宝石色边+0.9em 圆角+落影 0 4px 12px #07151e70；tp0-7 类型渐变（cards.css:16-23）；rv0-7 宝石色（:32-35，衍生青绿松石/棱彩幻彩锚 #cbb2f0）；cost 圆宝石 2.18em 左上 -0.48em 出血；art 窗 lib 5.8em/full 4.15em 径向底+家族位图 cover；name 深条 0.92em 粗体；type 行 0.66em「类型 · 稀有度」（职业=「XX专属」）；kw 药丸（抽 N/注能 N/回 N/甲 N，>0 才渲染）；gem 0.8em 菱形；desc 浅灰条 0.78em（空显「效果描述（可选）」）；dmg 左下六边形（DMG_TYPES 且 dmg>0 或负攻击）；val 右下金边圆（币值>0） |
| Lib 简卡模式 | winter.css .hs-card.lib(:217-219) | 隐藏 desc/type/kw、art 高 5.8em（图鉴网格一页 24 张瞬时渲染的网页口径） |
| 图鉴页 CodexPage | game.cardslib.js renderCardLibrary(:154-266) + cards.css .pg-head/.clib-*(:169-244) + winter.css 覆盖(:105-110,:217-221,:305-339) | 结构=pg-head（book 图标+「卡牌收藏」20px ls2px+搜索 210px 胶囊+稀有度下拉+「共 N 张」）→clib-tabs（全部+8 类型，计数角标 em 10px α.62）→clib-main（左预览 276px 深浅渐变+winter 右边框 #b7b4aa / 右 lib-grid）；winter lib-grid 固定 6 列 gap 0、lib-item padding 16px 12px 14px 分隔线 #bbb8ae；预览空态「把鼠标悬停在右侧卡牌上这里会显示大图预览」+pv-hint（type·稀有度+伤害词条+效果词条，cardslib.js:88-99）；悬停 90ms 去抖更新预览（:258 previewTimer）；翻页/筛选条件变化回第 1 页（:198-201,225-231）；lib-pager「‹ 上一页 / 第 N / M 页 · 共 X 张 / 下一页 ›」13px #5f5c53；页整体 winter #overlay .card.page 底 linear-gradient(125deg,#1b3645,#102431)。**差异登记**：①type-tab 胶囊在 winter 主题下无覆盖，取 hub-tab 同族色（idle 边 #4c6a76/on #d7c193 系）非石墨原色；②「＋制作新卡/导出/导入」按钮与卡牌设计器（openCardDesigner）不复刻——Godot 制作坊本批=碎片合成界面（任务口径），点卡=查看大图预览；③收藏态标记位=lib-item ✦（数据=[6b'→A] 收藏集合，到位即点亮） |
| 制作坊页 WorkshopPage | game.bag.js craftColorToken(:281-305)/chips(:513)/特写按钮(:269-271) + cards.css .pg.cdes/.cdes-*(:246-300) + winter.css:300-302 | 标题「卡牌制作坊」（cardslib.js:348 同名，winter h2 20px）；布局=左 cdes-stage（flex 1.15 径向底）产物=彩色令牌 cmtmvq6ss84l 全卡预览 + 右 cdes-form（min-width 400 滚动，cdes-sec 分区标题）；配方 1=3×员工通行证B(tt-token-gold)→1×通行证A(tt-token-color) 按钮「合成员工通行证A（3 张 B → 1 张 A）」（bag.js:269 文案）；配方 2=通行证A+2 枚碎片→彩色令牌 按钮「合成彩色令牌（2 碎片 + 1 通行证A）」（bag.js:271 文案）；材料行=res-chip 深色样式（碎片计数 N/2、袋子 chips「彩色令牌碎片 N/2」bag.js:513 同语义）；合成日志=StatusText 消费（adapter craft 动作返回消息）。动作=ICoreUiPort.RequestRunAction("craft:tokenA"/"craft:colortoken")（批次 4b 已落地，TokenCraft 3B→A、A+2碎片→彩色令牌语义） |
| hub 五页签框架 RunScreen | game.hub.js renderHub(:24-149) + css/hub.css(:3-40) + winter.css hub 覆盖 | TABS=出发(flag)/仓库(home)/升级(tools→gear 近似)/人物(medal)/成就·收藏室(trophy→lib 近似，资产无 trophy)；hub-tab 结构=tab-ico 42×42 圆角 10 内 24px 图标+tab-txt 12.5px ls3px；hub-tabs gap 10 padding-top 10；on 态 winter bg #d7c193 字 #16323e、idle #9db4bd；页签切换播 hubPageIn .3s cubic-bezier(.22,.61,.36,1) translateY 12px（仅切换时，hub.js tabChanged 口径）；hub-head res-chips（木材/口粮/钥匙/储备）；页内容区滚动（hub-page overflow-y auto padding 18px 22px） |
| 仓库页 | game.hub.js hubStashHTML(:450-503) | 左「卡牌仓库」=容量行「仓库容量 X / Y 张」+base-bar+stash-list（行=名×N+meta「N费·类型·收购 X 币/张」，sellPrice 口径=cards.js value 优先/稀有度半价；InventoryLabels 栈名→卡库反查）；右「消耗口袋」（钥匙复原说明：古朴1/稀有2/史诗3/传说4，下次出发清空）+「物资」行（木材/口粮/钥匙，材料不可卖币 2026-09-08 定版）；收藏态 ✦ 标记位=[6b'→A]。**差异登记**：卖出/收藏弹窗动作（openStashItem）需 [6b'→A] 仓库明细+动作快照后接入 |
| 升级页 | game.hub.js hubUpgradeHTML(:691-739) | 「背包扩建」「仓库扩建」两卡（容量行+bar+ov-btn.ok 扩建按钮，动作 RequestBaseAction upgrade:bag/upgrade:stash=批次 4b 接口）；按钮可用性吃快照 Actions[].Enabled（BuildBaseActions Detail「2 木材 · X/Y」解析容量）；「宠物升级」卡=说明态（口粮 2-3-4-5、Lv.5 上限、每级+1 保护格文案），数据待批次 5 |
| 人物页 | game.hub.js hubClassesHTML(:742-758) | 「人物熟练度」卡：ach-row 行=cls-hub-art 立绘 52×52+名+Lv 角标+说明+xp-bar；人物名册=data/characters.json（CharacterRoster，5 角色实数据）；熟练度 lv/xp/need=[6b'→A]（xp-bar 恒 0 过渡） |
| 成就·收藏室页 | game.hub.js hubAchHTML(:804-848)+collRoomHTML(:763-801) | 本批=页签框架空态：说明文案（卡背图鉴/成就/职业收藏室 收藏职业卡+10、能力卡+50、5/15/30/45/全收集里程碑）+「收藏档案为空」空态；数据消费待批次 5 快照（[6b'→A]） |
| 入口接线 | game.boot.js:422-423（btnCardLib→openCardLibrary、btnCardDesigner→openCardDesigner(null)） | MenuScreen ak-banner「制作坊」→OpenWorkshop（原挂点无点击）、ak-circle「收藏图鉴」→OpenCodex（原 onClick=null）；hub 入口=选档空档→Navigate("run")（对齐 newSlot→openBaseHub('deploy')，批次 6b 既有）；Esc 关闭页=MenuScreen 既有 closeTopOverlayByEsc 机制 |
| smoke/录帧参数 | —（Godot 侧取证通道） | --ui-codex 直开图鉴、--ui-workshop 直开制作坊（menu 屏）、--ui-hub-tab=deploy|stash|upgrade|classes|ach 直开 hub 对应页签（run 屏）；--shot=path.png 抓验收帧（UiScreen 既有） |
| 卡面插画家族 | art.js cardFamily(:58-76)/cardIcon(:238+) | 近似映射（本工程 assets/cards 仅 5 张家族位图）：能力卡→hero、武术→equipment-weapon（网页分 martial-melee/ranged）、法术→spell（网页分 spell/healing）、资源→resource-material、其余（生物/道具/装备/事件）→unknown；专属卡图（spell-*/martial-*/equip-*/creature-*/items/*）未随资产批次导入，完整 1:1 待美术资产批次 |

## wave6 批次 6b'' 收藏室页 + 宠物内容 ↔ 真源参数对照（2026-09-12）

| 组件/屏区 | 网页真源（搜打撤/game/src + css） | 取用参数（1:1 口径） |
|---|---|---|
| 职业收藏室 BuildAchPage | game.hub.js collRoomHTML(:763-801) + hub.css .coll-*(:160-184) | 标题「职业收藏室」+右上 set-tip「收藏进度 P / T」（P/T=RunUiSnapshot.Collection.Progress/Total，批次 5 快照）；说明文案全段照录（收藏只做记录+转化经验、卡牌保留在仓库、+10/+50 同张只计一次）；base-bar=CapacityBar 填充 #b8934c，值=P/T；总宽口径 hub-card（winter 浅色系，6b 已验收语言） |
| 五里程碑行 .coll-ms | collRoomHTML msRows(:768-779) + meta.js COLL_MILESTONES/claimColl(:133-186) | 行=「收藏 {Need} 张」粗体+奖励文案（快照 Reward=adapter collRewardText 同构「木材 ×3 · 口粮 ×5 · 钥匙 ×10 · 传说卡 ×2 · 宠物蛋 ×1」）+右侧三态：claimed→「✓ 已领取」绿 / reached→mini-btn.ok「领取」→RequestBaseAction($"collclaim:{Id}")（任务口径 collectclaim/RequestRunAction 以 adapter 实际为准，实测=collclaim: 走 RequestBaseAction，src/App/CoreGameAdapter.cs:321）/ 未达→「P / Need」dim；reach 态边框 rgba(185,147,76,.6)+底 #f3ead6、done 态 Modulate α=.72（.coll-ms.done opacity .72） |
| 熟练度转化条 | collRoomHTML 组头文案 + hubClassesHTML xp-bar(:749-750) + meta.js classSummary | 五列格=人物名+Lv 角标（maxed→「Lv.N · MAX」琥珀 #e0a458）+CapacityBar #6a6a6a+「经验 X / Y」（maxed→「已满级」）；数据=Collection.Classes（Lv/Xp/XpForNext/Maxed，meta.js classSummary 口径批次 5 交付）；节标题「熟练度转化 · 收藏职业卡 +10 / 能力卡 +50 经验（同一张只计一次）」（任务要求 +10/+50 转化语义展示） |
| 收藏池 66 张网格 | collRoomHTML groups(:780-792) + meta.js isCollectible/collectPool(:105-118) | 池=cls∈五职业 且（稀有度=职业 ∪ 类型=能力卡），UI 侧 CodexCatalog.Cards 同谓词过滤（node 复算 data=66：侠客13/战士13/牧师13/法师13/降临者14，与批次 5 CollectionRoom.Pool 实测一致）；组序=Collection.Classes（cards.json classes 序）→人物名经 CharacterRoster rulesetId 映射（coll-group-head characterName 口径）；组头=名+「gotN / N 张 · 收藏职业卡 +10 经验 · 能力卡 +50 经验」；GridContainer 13 列 gap 8（.coll-cards flex wrap gap 8 等效；13 张组单行排满）；on 槽=CardFace Lib fontPx 9（90×126，网页 hs-card 8px≈80×112 的同比例近似）+右上 ✦ #b8934c（CodexPage LibItem 同款标记位）；off 槽=90×126 圆角 9「？」22px |
| 宠物栏（仓库页） | game.hub.js hubPetsHTML(:506-537) + hub.css .pet-row(:271-277) | 卡题「宠物」+tip「N / 6 只 · 携带 1 只出战」；说明行全段照录（初始汪汪狗自动获得/蛋 0.7%+50 币仓库孵化/升级页口粮升级）；六行 .pk-row.pet-row：未孵化=「？？？ · 未孵化」+右「宠物蛋 + 50 币孵化」锁定 α=.55；已拥有=名+Lv.X+desc（去「携带效果：」前缀，web replace 同构）+右「✓ 携带中」绿 /「携带」mini-btn.ok→RequestBaseAction($"pet:sel:{Id}")；on 态=边 rgba(255,202,91,.55)+底 rgba(255,202,91,.08)（.pet-row.on 原值）；孵化按钮=「孵化宠物（-1 蛋 -50 币）」ov-btn.ok→RequestBaseAction("pet:hatch")，Disabled=快照 Actions["pet:hatch"].Enabled（adapter 已含 有蛋+币足+未集齐 三条件）；hint 两态=孵化就绪「仓库里有宠物蛋——…」/未就绪「孵化需要宠物蛋 ×1 + 50 币（当前储备 N 币…）」（web hasEgg ok-hint 语义） |
| 宠物升级行（升级页） | game.hub.js hubUpgradeHTML(:690-739) 宠物段 | 卡题「宠物升级」+tip「口粮 2-3-4-5 · 携带中的宠物决定保护格 N 格」（N=快照 SafeCapacity，RunBaseState 安全格=携带宠等级+safeBonus 批次 5 语义）；六行：未孵化=「？？？ · 未孵化（宠物蛋 + 50 币，仓库页）」+「Lv.? / 5」锁定；已拥有=名+「✓ 携带中」+desc（ClipText 防挤）+「Lv.N · MAX / Lv.N → N+1」+按钮 maxed→「已满级」disabled /「口粮 ×{UpgradeCost} 升级」disabled=!CanUpgrade→RequestBaseAction($"pet:up:{Id}")；Lv 上限 5=pets.json levelMax（快照无 LevelMax 字段，UI 常量 5 注释标注） |
| 宠物图标 icon-paw.png | hubPetsHTML [[icon:paw]] / pets.json icon 字段 | 资产库无 paw/宠物位图——自绘 48×48 爪印 png（#6b685b 四趾+主垫，hub 卡文字同族色）；png raw 直装模板 pck 可载（6c-map 管线验证）；pets.json 逐宠 icon（runner/coin/tools/fire/crystal）未透出快照且无对应资产，统一 paw，**差异登记** |
| 卡背图鉴/成就陈列（空态） | game.hub.js hubAchHTML(:804-848) | 网页=卡背网格（CARD_BACKS/解锁/装备）+17 成就行（unlocked/claimed/reward）+收藏室三段；Godot 本批只落收藏室（任务口径），卡背/成就=空态置灰说明行——解锁·领取状态集合快照未透出（[6b'→A] 8 扩展，归 A 线批次 8 前置），不做假数据渲染 |
| smoke/录帧参数 | —（Godot 侧取证通道） | --ui-hub-tab=deploy|stash|upgrade|classes|ach + 新增 --ui-hub-scroll=<px>（取证滚动偏移，_Process 延 10 帧应用后复位；6c-map --map-demo 同款模式）；录帧=模板 exe --write-movie <ASCII 路径>/f.png --quit-after 40 ++ …（write-movie 写中文相对路径报 d.is_null() FAILED，须 ASCII 绝对路径，帧序 f000000NN.png 取后段） |
